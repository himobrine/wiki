...
### `2026 // 05 / 28` [**OWASP Top 10 深度解读：注入攻击 (Injection)**](posts/owasp-injection.html)
...
> 注入攻击连续多年位居 OWASP Top 10 榜首。无论是 SQL 注入、NoSQL 注入还是 OS 命令注入，其本质都是**用户输入被解释器当作代码执行**。

[`[ + ] READ MORE →`](posts/owasp-injection.html)

---

### `2026 // 05 / 25` [**渗透测试工具链：从信息收集到权限维持**](posts/pentest-toolchain.html)
...
> 一次完整的渗透测试通常遵循 **PTES (Penetration Testing Execution Standard)** 标准流程。本文整理各阶段的核心工具及使用技巧。

[`[ + ] READ MORE →`](posts/pentest-toolchain.html)

---

### `2026 // 05 / 20` [**CTF 学习笔记：Web 安全入门实战**](posts/ctf-web-intro.html)
...
[`[ + ] READ MORE →`](posts/ctf-web-intro.html)

---

### `2026 // 05 / 15` [**从 Log4j 漏洞看供应链安全**](posts/log4j-analysis.html)
...
> CVE-2021-44228 (Log4Shell) 是近年来影响最广泛的漏洞之一。一个几乎无处不在的日志库，一个 JNDI 注入，导致了整个互联网的"大地震"。

[`[ + ] READ MORE →`](posts/log4j-analysis.html)
