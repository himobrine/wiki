...
var spacer=null;
if(sentinel&&navWrap){
  var navObs=new IntersectionObserver(function(e){var stuck=!e[0].isIntersecting;navWrap.classList.toggle('fixed',stuck);if(stuck&&!spacer){spacer=document.createElement('div');spacer.style.height=navWrap.offsetHeight+'px';navWrap.parentNode.insertBefore(spacer,navWrap.nextSibling);}else if(!stuck&&spacer){spacer.remove();spacer=null;}},{threshold:[0]});
  navObs.observe(sentinel);
}

var animObserver=new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting){e.target.classList.add('visible');animObserver.unobserve(e.target);}});},{threshold:0.1});
document.querySelectorAll('.fade-up').forEach(function(el){animObserver.observe(el);});

var html=document.documentElement;
var themeBtn=document.getElementById('themeToggle');
var themeDock=document.getElementById('themeDock');
if(html&&themeBtn&&themeDock){
  var themeIcons={dark:'☽',light:'☀',crystal:'◇',ego:'♢',rei:'レ'};
  var isTouch='ontouchstart' in window||navigator.maxTouchPoints>0;
  var qToggle='light';
  function loadThemeCSS(t){
    var existing=document.getElementById('theme-css');
    if(existing)existing.remove();
    if(t==='dark')return;
    var path='assets/theme-'+t+'.css';
    if(window.location.pathname.indexOf('/tags/')!==-1||window.location.pathname.indexOf('/posts/')!==-1)path='../assets/theme-'+t+'.css';
  ...
  function setTheme(t){
    ...
    document.querySelectorAll('.theme-dock-item').forEach(function(el){el.classList.toggle('active',el.dataset.theme===t);});
    loadThemeCSS(t);
    if(t==='dark')qToggle='light';
    else if(t==='light')qToggle='dark';
    else if(t==='rei')qToggle='dark';
    else qToggle='dark';
  }
  var saved=localStorage.getItem('theme')||'dark';
  setTheme(saved);
  if(saved==='dark'||saved==='light')qToggle=saved==='dark'?'light':'dark';
  else if(saved==='rei')qToggle='dark';
  document.querySelectorAll('.theme-dock-item').forEach(function(el){
    el.addEventListener('click',function(){setTheme(el.dataset.theme);themeDock.classList.remove('visible');});
  });
...
if(topBtn){
  ...
  var topObs=new IntersectionObserver(function(e){topBtn.classList.toggle('show',!e[0].isIntersecting);},{threshold:[0]});
  var sentinelEl=document.getElementById('navSentinel');
...
/* === Image Lightbox (click → zoom, wheel → scale, pinch → zoom, drag → pan) === */
(function(){
